...
const CONFIG = {
  ...
  htmlFiles: ['index.html', 'mentoring.html', 'proofreading.html', 'showcase.html'],
  minifyOptions: {
  ...
  try {
    for (const filename of CONFIG.htmlFiles) {
      ...
      if (result) {
        ...
        console.log(`✓ ${result.filename}: ${formatBytes(result.originalSize)} → ${formatBytes(result.minifiedSize)} (${result.percent}% saved)\n`);
      }
    }

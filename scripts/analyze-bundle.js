...
const CONFIG = {
  ...
  filesToAnalyze: ["index.html", "output.css", "main.js", "main.min.js"],
  thresholds: {
    html: 50 * 1024, // 50KB
    css: 100 * 1024, // 100KB
    js: 150 * 1024, // 150KB
...
async function analyzeBundle() {
  ...
  results.forEach((result) => {
    const gzipPercent = (
      (result.gzipSavings / result.originalSize) *
      100
    ).toFixed(1);
    const brotliPercent = (
      (result.brotliSavings / result.originalSize) *
      100
    ).toFixed(1);
    ...
    console.log(
      `  Gzipped: ${formatBytes(result.gzipSize)} (${gzipPercent}% smaller)`
    );
    console.log(
      `  Brotli: ${formatBytes(result.brotliSize)} (${brotliPercent}% smaller)`
    );
    console.log("");
  });
  ...
  console.log(
    `Total Gzipped: ${formatBytes(totalGzip)} (${gzipPercent}% smaller)`
  );
  console.log(
    `Total Brotli: ${formatBytes(totalBrotli)} (${brotliPercent}% smaller)`
  );

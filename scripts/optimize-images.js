...
async function getImageFiles(dir) {
  ...
  async function traverse(currentDir) {
    ...
    for (const entry of entries) {
      ...
      } else if (entry.isFile() && /\.(jpe?g|png|svg|webp)$/i.test(entry.name)) {
        files.push(fullPath);
      }
    }
  }
...
async function optimizeImage(inputPath, outputPath) {
  try {
    ...
    console.log(`✓ ${path.basename(inputPath)} - ${formatBytes(originalSize)} → ${formatBytes(optimizedSize)} (${percent}% saved)`);
...
async function createWebPVersion(inputPath, outputDir) {
  try {
    ...
    console.log(`ℹ WebP creation skipped for ${filename} (requires sharp or imagemin-webp)`);
